...
int main( )
{
	...
	//clock_t Start, Time;
	FILE *fp_inorder=fopen("AVLHash_inorder.out","w");
	FILE *fp_postorder=fopen("AVLHash_postorder.out","w");
	...
	for(i=0;i<S;i++)
	{
		//printf("\nAVLHash_inorder.out [bucket %d]:\n",i);
		at[i].display_inorder(avl[i],fp_inorder);
		fprintf(fp_inorder,"\n");
		//printf("\nAVLHash_postorder.out [bucket %d]:\n",i);
		at[i].display_postorder(avl[i],fp_postorder);
